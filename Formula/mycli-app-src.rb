...
class MycliAppSrc < Formula
  ...
  def install
    ...
    # This follows the AWS CLI pattern for handling awscrt
    venv.pip_install resources.reject { |r| r.name == "pymsalruntime" }

    # Install pymsalruntime binary wheel separately (AWS CLI-style approach)
    # This allows Homebrew to handle the binary wheel correctly
    if resources.any? { |r| r.name == "pymsalruntime" }
      venv.pip_install resource("pymsalruntime")
    end
